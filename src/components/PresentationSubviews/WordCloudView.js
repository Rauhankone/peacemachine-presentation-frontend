...
`
  ...
  componentWillMount() {
    this.dom = ReactFauxDom.createElement(`div`);
  }
  ...
  render() {
    d3
      ...
      .selectAll(`*`)
      .remove();
    let fullText = this.state.data.utterances.map(w => w.text).join(` `);
    ...
    let data = keywords
      .map((w, i) => ({
        text: w,
        fontSize: Math.trunc(scale(relFreqs[i]))
      }))
    ...
    let layout = cloud()
      ...
      .on(`end`, words => {
        d3
          ...
          .style(`fill`, (d, i) => d3.scaleOrdinal(d3.schemeCategory20)(i))
